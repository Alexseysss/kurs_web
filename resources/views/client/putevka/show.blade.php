@extends('client.layouts.main')
@section('content')
    <!-- Content Wrapper. Contains page content -->
    <div class="content-wrapper">
        <!-- Content Header (Page header) -->
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6 d-flex align-items-center">
                        <h1 class="m-0 mr-2">Путёвка: {{$putevka->id}}</h1>
                        <a href="{{route('client.putevka.edit', $putevka->id)}}" class="text-success"><i class="nav-icon fas fa-pen"></i></a>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="{{route('client.main.index')}}">Главная</a></li>
                            <li class="breadcrumb-item active">Путёвки</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
        </div>
        <!-- /.content-header -->

        <!-- Main content -->
        <section class="content">
            <div class="container-fluid">
                <!-- Small boxes (Stat box) -->
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body table-responsive p-0">
                                <table class="table table-hover text-nowrap">
                                    <tbody>
                                        <tr>
                                            <td>ID</td>
                                            <td>{{$putevka->id}}</td>
                                        </tr>
                                        <tr>
                                            <td>Пользователь</td>
                                            <td>{{$putevka->user->name ?? ''}} {{$putevka->user->surname  ?? ''}}</td>
                                        </tr>
                                        <tr>
                                            <td>Статус</td>
                                            <td>{{$putevka->status}}</td>
                                        </tr>
                                        <tr>
                                            <td>Комната</td>
                                            <td>{{$putevka->room->number ?? ''}}
                                                <li><a href="{{route('client.room.show', $putevka->room->id ?? '')}}">Посмотреть</a></li>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>Дата покупки</td>
                                            <td>{{$putevka->date_buy}}</td>
                                        </tr>
                                        <tr>
                                            <td>Дата заезда</td>
                                            <td>{{$putevka->date_in}}</td>
                                        </tr>
                                        <tr>
                                            <td>Дата отъезда</td>
                                            <td>{{$putevka->date_out}}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
        </section>
        <!-- /.content -->
    </div>
    <!-- /.content-wrapper -->
@endsection
